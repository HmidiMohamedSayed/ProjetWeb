...
if($_POST['password']==$_POST['confirmpassword'] && verif($_POST['username'])==true ){
    $workerRepository = new WorkerRepository();
$workerRepository->ajouterWorker($_POST['fullname'],$_POST['username'],$_POST['email'],$_POST['phone'],$_POST['password'],$_POST['service'],$_POST['gender']);
header("location:Login.php");}
else if(verif($_POST['username'])==false){
    session_start();
    $_SESSION['exist']="Username exists Please try another one";
    header("location:Signupworker.php");
}
else{
    session_start();
